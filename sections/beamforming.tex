...
the John A. Galt 26\,m, and the Green Bank 140\,ft telescope. This would
allow for the first ever VLBI detection of an FRB. 
If the array consisted of just the DRAO-ARO baseline, 
we would instantaneously get milli-arcsecond resolution in one dimension,
but poor localization in the orthogonal direction. However, 
this could be improved upon from the earth rotation during 
the $\sim$30 seconds of dispersion delay. That said, 
a VLBI array with three or more elements 
would be ideal. 
...
\caption[abc]{A visual demonstration of the difference between 
     ...
     of the 3,074\,km DRAO-ARO baseline in our band.}
\label{fig-m51}
\end{center}
\end{figure}
...
Finally, millisecond intensities could be written to disk and stored
...
beam is not well calibrated in absolute flux density, this pulsar 
...
The ARO's 46\,m antenna has been mounted with a 
...
\subsubsection{A flat brightness distribution?}
...
as might be expected in the cosmological FRB scenario, then 
...
Using our non-detection in $\sim$1.5 months of observing, we can 
...
the minimum flux density to which we are sensitive is 
increased as,
...
We include this effect in our estimates in Fig.~\ref{fig-pf_alpha}. 
...
dumps baseband data to disk with 1024$\times$391\,kHz channels 
